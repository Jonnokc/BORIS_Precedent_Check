...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  ...
  ' inititalizes progress bar to 0
  ufProgress.LabelProgress.Width = 0
  For i = 1 To UBound(All_Prev_Mapped_Displays)
    ' forces progress bar to screen
    ufProgress.Show
    pcstdone = i / UBound(All_Prev_Mapped_Displays)
    ' cleans format, removes special characters, orders string alphabetically
    All_Prev_Mapped_Displays(i, 1) = Utils.Sort_Sub_Strings(Utils.Cleaning(Utils.ReplaceSplChars(All_Prev_Mapped_Displays(i, 1))))
    ' updates progress bar
    With ufProgress
        .LabelCaption = "Processing Row " & i & " of " & UBound(All_Prev_Mapped_Displays)
        .LabelProgress.Width = pcstdone * (.FrameProgress.Width)
    End With
    DoEvents
    ' closes progress bar when done
    If i = UBound(All_Prev_Mapped_Displays) Then
        Unload ufProgress
    End If
  Next i
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  ...
  ' Cell value currently looking for a closest match with
    ...
    DoEvents
    Application.StatusBar = "Current progress: " & i & " of " & UBound(Final_All_Unmapped_Displays_To_Check)

  ' Medication Checker. Looks to see if code is a legit medication, or not a medication at all.
    Medication_Check = Utils.Medication_Checker(Prev_Mapped_Checking, All_Prev_Medication_Displays, Unmapped_Code_System_To_Check)
